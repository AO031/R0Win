...
NTKERNELAPI NTSTATUS NTAPI ZwQueryInformationProcess(HANDLE ProcessHandle,PROCESSINFOCLASS ProcessInformationClass,PVOID ProcessInformation,ULONG ProcessInformationLength,PULONG ReturnLength);
...
NTKERNELAPI NTSTATUS PsReferenceProcessFilePointer(PEPROCESS Process, PVOID* OutFileObject);
...
NTSTATUS FindProcess(PEPROCESS *outProcess, PHANDLE outPid) {
    ...
    if (!NT_SUCCESS(st)) {
        DbgPrint("[E] ZwQuerySystemInformation Failed->0x%lX\n", st);
        goto ret;
    }
    ...
    while (TRUE) {
        ...
        if ((SIZE_T)pid > 4 && processInfo->ImageName.Buffer) {
            st = PsLookupProcessByProcessId(pid, &processObject);
            if (NT_SUCCESS(st)) {
                st = PsReferenceProcessFilePointer(processObject, &fileObject);
                if (NT_SUCCESS(st)) {
                    *outPid = pid;
                    *outProcess = processObject;
...
NTSTATUS TestPsLookupProcessByProcessId() {
	NTSTATUS st = STATUS_SUCCESS;
	PEPROCESS processObject = NULL;
	HANDLE currentPid = 0;
	...
	st = PsLookupProcessByProcessId(currentPid, &processObject);
	if (!NT_SUCCESS(st)) {
		DbgPrint("[E] PsLookupProcessByProcessId Failed->0x%lX\n", st);
		goto ret;
	}

	processName = PsGetProcessImageFileName(processObject);
	DbgPrint("[W] TestPsLookupProcessByProcessId Success,Process Name->%s\n", processName);

ret:
    if (processObject) ObDereferenceObject(processObject);
	return st;
}

NTSTATUS TestReferenceObjectByHandle() {
	NTSTATUS st = STATUS_SUCCESS;
	PEPROCESS processObject = NULL;
	...
	st = ObReferenceObjectByHandle(processHandle, PROCESS_ALL_ACCESS, *PsProcessType, KernelMode, &processObject,NULL);
	if (!NT_SUCCESS(st)) {
		DbgPrint("[E] ObReferenceObjectByHandle Failed->0x%lX\n", st);
		goto ret;
	}

	processName = PsGetProcessImageFileName(processObject);
	DbgPrint("[W] TestReferenceObjectByHandle ProcessName->%s\n", processName);
...
NTSTATUS TestOpenObjectByPointer() {
	NTSTATUS st = STATUS_SUCCESS;
	PEPROCESS processObject = NULL;
	HANDLE currentPid = 0;
	HANDLE processHandle = 0;
	...
	st = PsLookupProcessByProcessId(currentPid, &processObject);
	if (!NT_SUCCESS(st)) {
		DbgPrint("[W] PsLookupProcessByProcessId Failed->0x%lX\n", st);
		goto ret;
	}
	...
	st = ObOpenObjectByPointer(
		...
		*PsProcessType,
		...
		&processHandle);
	if (!NT_SUCCESS(st)) {
		DbgPrint("[W] ObOpenObjectByPointer Failed->0x%lX\n", st);
		goto ret;
	}

	DbgPrint("[W] TestOpenObjectByPointer Process Handle->%p\n", processHandle);
...
NTSTATUS TestPsGetProcessImageFileName() {
	NTSTATUS st = STATUS_SUCCESS;
	PEPROCESS processObject = NULL;
	HANDLE currentPid = 0;
	PCHAR processName = NULL;
	...
	processName = PsGetProcessImageFileName(processObject);
	DbgPrint("[W] PsGetProcessImageFileName->%s\n", processName);
...
NTSTATUS TestQueryProcessImagePath() {
	NTSTATUS st = STATUS_SUCCESS;
	...
	if (!NT_SUCCESS(st)) {
		DbgPrint("[E] ZwQueryInformationProcess Failed->0x%lX\n", st);
		goto ret;
	}
	filePath = (PUNICODE_STRING)buffer;

	DbgPrint("[W] ZwQueryInformationProcess->%wZ\n", filePath);
...
NTSTATUS TestPsReferenceProcessFilePointer() {
    PEPROCESS processObject = NULL;
    HANDLE pid = 0;
    NTSTATUS st = STATUS_SUCCESS;
    PFILE_OBJECT fileObject = NULL;
    ...
    st = FindProcess(&processObject, &pid);
    if (!NT_SUCCESS(st)) {
        DbgPrint("[E] FindProcess Failed->0x%lX\n", st);
        goto ret;
    }
    
    st = PsReferenceProcessFilePointer(processObject, &fileObject);
    if (!NT_SUCCESS(st)) {
        DbgPrint("[E] PsReferenceProcessFilePointer Failed->0x%lX\n", st);
        goto ret;
    }
    ...
    DbgPrint("[W] PsReferenceProcessFilePointer->%wZ\n", &(nameInfo->Name));

    st = IoQueryFileDosDeviceName(fileObject, &dosNameInfo);
    if (!NT_SUCCESS(st)) {
        DbgPrint("[E] IoQueryFileDosDeviceName Failed->0x%lX\n", st);
        goto ret;
    }
    DbgPrint("[W] IoQueryFileDosDeviceName->%wZ\n", &(dosNameInfo->Name));

ret:
    if (dosNameInfo) ExFreePool(dosNameInfo);
    if (buffer) ExFreePool(buffer);
...
NTSTATUS TestSeLocateProcessImageName() {
    NTSTATUS st = STATUS_SUCCESS;
    PEPROCESS processObject = NULL;
    ...
    st = SeLocateProcessImageName(processObject,&imageFileName);
    if (!NT_SUCCESS(st)) {
        DbgPrint("[E] SeLocateProcessImageName Failed->0x%lX\n", st);
        goto ret;
    }

    DbgPrint("[W] SeLocateProcessImageName->%wZ\n", imageFileName);
...
NTSTATUS TestPsGetProcessId() {
    ...
    tempPid = PsGetProcessId(processObject);
...
NTSTATUS TestGetParentProcessId() {
    NTSTATUS st = STATUS_SUCCESS;
    PEPROCESS processObject = NULL;
    HANDLE parentPid = 0;
    ...
    // What? You say FromUniqueProcessId but need Process Object?
    parentPid = PsGetProcessInheritedFromUniqueProcessId(processObject);
    DbgPrint("[W] PsGetProcessInheritedFromUniqueProcessId:%d\n", parentPid);
...
NTSTATUS TestGetProcessInfomartion() {
    HANDLE currentPid = 0;
    NTSTATUS st = STATUS_SUCCESS;
    PEPROCESS processObject = NULL;
    ...
    currentTime = PsGetProcessCreateTimeQuadPart(processObject);
    if (currentTime) {
        systemTime.QuadPart = currentTime;
        ExSystemTimeToLocalTime(&systemTime, &localTime);
        RtlTimeToTimeFields(&localTime, &timeFields);
        DbgPrint("[W] CreateTime %04d-%02d-%02d %02d:%02d:%02d\n",
            timeFields.Year, timeFields.Month, timeFields.Day,
            timeFields.Hour, timeFields.Minute, timeFields.Second);
    }

    exitStatus = PsGetProcessExitStatus(processObject);
    if (exitStatus == STATUS_PENDING) {
        DbgPrint("[W] Process Is Runnning\n");
    }
    else {
        DbgPrint("[E] Process Is Not Running\n");
    }
    
    sessionId = PsGetProcessSessionId(processObject);
    if (sessionId == 0) {
        DbgPrint("[W] System Service Session\n");
    }
    else {
        DbgPrint("[E] User Session\n");
    }

    isDebugged = PsIsProcessBeingDebugged(processObject);
    if (isDebugged) {
        DbgPrint("[E] Is Debugged\n");
    }
    else {
        DbgPrint("[W] Not Being Debugged\n");
    }
...
NTSTATUS TestAttachProcess() {
    NTSTATUS st = STATUS_SUCCESS;
    PEPROCESS targetProcess = NULL;
    KAPC_STATE apcState = { 0 };
    HANDLE currentPid = (HANDLE)7504;
    PVOID targetAddress = (PVOID)0x400000;
    UCHAR buffer[16] = { 0 };

    st = PsLookupProcessByProcessId(currentPid, &targetProcess);
    if (!NT_SUCCESS(st)) {
        DbgPrint("[E] PsLookupProcessByProcessId Failed->0x%lX\n", st);
        goto ret;
    }
    ...
    __try {
        ProbeForRead(targetAddress, sizeof(buffer), 1);

        RtlCopyMemory(buffer, targetAddress, sizeof(buffer));
    ...
    __except (EXCEPTION_EXECUTE_HANDLER) {
        DbgPrint("[E] Exception In Reading Momory\n");
    }
...
NTSTATUS DriverEntry(PDRIVER_OBJECT driverObject, PUNICODE_STRING regPath) {
	...
	// TestPsLookupProcessByProcessId();
	// TestReferenceObjectByHandle();
	// TestOpenObjectByPointer();
	// TestPsGetProcessImageFileName();
	// TestQueryProcessImagePath();
    // TestPsReferenceProcessFilePointer();
    // TestSeLocateProcessImageName();
    // TestPsGetProcessId();
    // TestGetParentProcessId();
    // TestGetProcessInfomartion();
    TestAttachProcess();
